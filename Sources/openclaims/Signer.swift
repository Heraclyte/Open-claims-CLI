...
struct Signer {
    ...
    private static func loadPrivateKey(from data: Data) throws -> Curve25519.Signing.PrivateKey {
        ...
        let rawBytes = try extractRawBytesFromPEM(data: data, prefix: "PRIVATE KEY")

        // Validate that this is exactly a 48-byte PKCS#8 Ed25519 key by checking the OID prefix
        guard rawBytes.count == 48, rawBytes.prefix(16).elementsEqual(ed25519PrivateKeyPrefix)
        else {
            throw NSError(
                domain: "SignerError",
                code: 1,
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Invalid key format. Expected a raw 32-byte Ed25519 private key or a valid PKCS#8 PEM/DER file."
                ]
            )
        }
    ...
    private static func loadPublicKey(from data: Data) throws -> Curve25519.Signing.PublicKey {
        ...
        // Strip PEM headers/footers and whitespaces if it's an OpenSSL PEM file
        let rawBytes = try extractRawBytesFromPEM(data: data, prefix: "PUBLIC KEY")
        ...
        guard rawBytes.count == 44, rawBytes.prefix(12).elementsEqual(ed25519PublicKeyPrefix) else {
            throw NSError(
                ...
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Invalid key format. Expected a raw 32-byte Ed25519 public key or a valid SPKI PEM/DER file."
                ]
            )
        }
    ...
    private static func extractRawBytesFromPEM(data: Data, prefix: String) throws -> Data {
        guard let text = String(data: data, encoding: .utf8) else {
            throw NSError(
                domain: "SignerError", code: 3,
                userInfo: [NSLocalizedDescriptionKey: "Cannot read key as UTF-8 string"])
        }
        let cleaned =
            text
        ...
        guard let decodedData = Data(base64Encoded: cleaned) else {
            throw NSError(
                domain: "SignerError", code: 4,
                userInfo: [NSLocalizedDescriptionKey: "Invalid Base64 format in PEM file"])
        }
